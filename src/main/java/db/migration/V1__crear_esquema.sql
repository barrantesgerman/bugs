...
CREATE TABLE `incidencia` (
  ...
  `usuario_asignacion` varchar(50) DEFAULT NULL,
  `fecha_asignacion` datetime DEFAULT NULL,
  `usuario_atension` varchar(50) DEFAULT NULL,
  `fecha_atension` datetime DEFAULT NULL,
  `usuario_resolucion` varchar(50) DEFAULT NULL,
  `fecha_resolucion` datetime DEFAULT NULL,
  `usuario_revision` varchar(50) DEFAULT NULL,
...
`inc`.`usuario_creacion` AS `usuario_creacion`,
`inc`.`fecha_creacion` AS `fecha_creacion`,
`inc`.`usuario_actualizacion` AS `usuario_actualizacion`,
`inc`.`fecha_actualizacion` AS `fecha_actualizacion`,
`inc`.`usuario_asignacion` AS `usuario_asignacion`,
`inc`.`fecha_asignacion` AS `fecha_asignacion`,
`inc`.`usuario_atension` AS `usuario_atension`,
`inc`.`fecha_atension` AS `fecha_atension`,
`inc`.`usuario_resolucion` AS `usuario_resolucion`,
`inc`.`fecha_resolucion` AS `fecha_resolucion`,
`inc`.`usuario_revision` AS `usuario_revision`,
`inc`.`fecha_revision` AS `fecha_revision`,
`inc`.`usuario_cierre` AS `usuario_cierre`,
